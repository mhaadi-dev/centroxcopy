...
export async function GET(request: NextRequest) {
  ...
  if (isNaN(startRange) || isNaN(endRange) || startRange < 0 || endRange <= startRange) {
    return NextResponse.json(
      { error: 'Invalid range parameters. Ensure startRange and endRange are valid and endRange > startRange.' },
      { status: 400 }
    );
  }
...
export async function POST(request: NextRequest) {
    try {
      ...
      const blogData = await client.fetch(PAGINATED_SEARCH_BLOGS_QUERY(keyword,startRange,endRange));
      const totalBlogs=await client.fetch(TOTAL_SEARCH_BLOGS_COUNT_QUERY(keyword))
