...
// Define the solutions object type
interface Solutions {
  snapandmeasure: string;
  labelling: string;
  cryptopredictor: string;
  gymbuddy: string;
  healthbot: string;
}

// Define valid solution keys
type SolutionKey = keyof Solutions;

// Define solutions object with explicit typing
const solutions: Solutions = {
  snapandmeasure: process.env.NEXT_PUBLIC_SNAP_DEMO || "",
  labelling: process.env.NEXT_PUBLIC_LABELLING_DRESSES || "",
  cryptopredictor: process.env.NEXT_PUBLIC_CRYPTO_PREDICTOR || "",
  gymbuddy: process.env.NEXT_PUBLIC_GYM_BUDDY || "",
  healthbot: process.env.NEXT_PUBLIC_HEALTH_BOT || "",
};

// Component props with typed solution
interface SolutionIntegrationProps {
  solution?: SolutionKey;
}

const SolutionIntegration = ({ solution = "gymbuddy" }: SolutionIntegrationProps) => {
      ...
      const formResponse = await fetch("/api/submit-form", {
        ...
        body: JSON.stringify({
          fields: [
            ...
            {
              name: "Subject",
              value: "Accessing Demo of Gym Buddy Solution",
            },
            {
              name: "Email",
              value: email,
  ...
  const redirectToDemo = () => {
    if (solution && solutions[solution]) {
      debugger;
      window.open(solutions[solution], "_blank");
    }
  ...
  if (showDemo && demoToken) {
    return (
      <Button
        className="mx-auto my-10 lg:my-24"
        content="Go to Demo"
  ...
  if (showSuccess) {
    return (
      <section
        className={classNames(section_wrapper_class, "flex items-center justify-center")}
      >
  ...
  return (
    <section className={classNames(section_wrapper_class)}>
      <header aria-label="Centrox Contact" className="flex flex-col gap-y-4">
        ...
        <p
          className={classNames(
            text_para_2,
            "text-center capitalize w-full sm:w-[60%] mx-auto"
          )}
        >
      ...
      {!showOtpInput && (
        <div className="flex flex-col gap-y-0 lg:gap-y-0 lg:flex-row gap-x-5">
          <input
            type="text"
            name="name"
      ...
      {!showOtpInput ? (
        <Button
          ...
          isDisabled={loading}
          className={classNames(
            "w-full sm:w-[70%] xl:w-[50%] my-8 mx-auto sm:!px-[1.4rem] 2xl:!px-[2rem] sm:!py-[1rem] 2xl:!py-[1.15rem]"
          )}
        />
      ) : (
        <button
